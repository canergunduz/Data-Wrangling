...
def update_name(name, mapping): 
    """
    Change road names according to the mapping above 
    Modify all the alphabet avenues
		Original: 
							'Avenue L'
		Returns:
							'L Avenue'
...
clean_postal = re.compile(r'^(\d{5})')
new = re.compile(r'^\D*(\d{5})')
    ...
    if element.tag == 'way' or element.tag == 'node':
        for sub in element:
            if sub.get('k') == 'addr:postcode':
                new_zip = new.match(sub.attrib['v'])
                zip_codes = clean_postal.match(sub.attrib['v'])
                if new_zip:
                    if zip_codes:
                        if sub.attrib['v'] not in zip_codes.group():
                            sub.attrib['v'] = new.match(sub.attrib['v']).group(1)
                    else:
                        sub.attrib['v'] = new.match(sub.attrib['v']).group(1)
...
def shape_element(elem):
    """
		Form a node for each entry
		Organize different types of info into dict format
		"""
